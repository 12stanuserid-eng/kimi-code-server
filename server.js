...
window.ksCf=function(){document.getElementById("ks-id").value="";document.getElementById("ks-url").value="";document.getElementById("ks-key").value="";document.getElementById("ks-typ").value="openai";document.getElementById("ks-em").textContent="";};
window.ksSt=function(m,t){var x=document.getElementById("ks-st");x.textContent=m;x.className=t||"ks-ok";x.style.display="block";if(t!=="ks-wait")setTimeout(function(){x.style.display="none";},4000);};
...
window.ksE=function(id,type,url){document.getElementById("ks-id").value=id;document.getElementById("ks-url").value=url;document.getElementById("ks-key").value="";document.getElementById("ks-typ").value=type||"openai";document.getElementById("ks-em").textContent="Editing: "+id+" (leave key blank to keep)";};
window.ksS=function(){var id=document.getElementById("ks-id").value.trim();var url=document.getElementById("ks-url").value.trim();var key=document.getElementById("ks-key").value.trim();var typ=document.getElementById("ks-typ").value;if(!id||!url){ksSt("ID and URL required","ks-bad");return;}var b=JSON.stringify({id:id,type:typ,base_url:url,api_key:key});fetch("/kimi-admin/providers",{method:"POST",headers:{"Content-Type":"application/json"},body:b}).then(function(r){return r.json();}).then(function(d){if(d.success){ksSt("Saved! Restart to apply.","ks-ok");ksRef();ksCf();}else{ksSt("Error: "+(d.error||"?"),"ks-bad");}}).catch(function(e){ksSt("Error: "+e.message,"ks-bad");});};
