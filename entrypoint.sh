#!/bin/bash
# Entrypoint: starts PostgreSQL via Debian cluster management, then pentaract
...
# Clean up any existing cluster and create a fresh one using Debian tools
echo "[entrypoint] Setting up PostgreSQL cluster (Debian way)..."
...
pg_createcluster --start 15 main 2>&1 || {
    ...
    # Configure for local password auth
...
# Ensure password auth for TCP connections (pg_createcluster default is scram-sha-256)
# The pentaract binary connects via TCP with password
